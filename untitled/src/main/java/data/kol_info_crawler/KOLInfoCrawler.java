...
public class KOLInfoCrawler extends Crawler {
      ...
      private int getProfilesData(List<WebElement> profiles) {
            ...
            for (WebElement profile: profiles) {
                  ...
                  // NOTE: because the page is manually loaded, we can only process around half
                  // the number of divs rendered (around 1/2 * 30 = 15), so we set
                  // MAX_PROFILE_PER_SESSION to be 14 and stop when total_count reach the limit.
                  if(total_count == MAX_PROFILE_PER_SESSION) break;
